...
def load_models(device):
    gmm = GMM().to(device)
    alias = ALIASGenerator().to(device)
    load_checkpoint(gmm, os.path.join(CHECKPOINT_DIR, "gmm.pth"))
    load_checkpoint(alias, os.path.join(CHECKPOINT_DIR, "alias.pth"))
    gmm.eval()
    alias.eval()
...
if person_image and cloth_image:
    person_np = preprocess_image(person_image)
    cloth_np = preprocess_image(cloth_image)
    st.image([person_np, cloth_np], caption=["Person", "Clothing"], width=200)
    ...
    if st.button("Generate Try-On Image"):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        st.info("Loading models...")
        gmm, alias = load_models(device)
        
        st.info("Running Virtual Try-On...")
        tester = VirtualTryOnTester(gmm, alias, device)
        output_img = tester.run(person_np, cloth_np)
